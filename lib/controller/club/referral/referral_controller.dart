...
class ReferralController extends GetxController {
  ...
  final referralModel = Rx<ReferralModel?>(null);
  ...
  Future<void> fetchReferralDetails({
    ...
    try {
      ...
      final res = await http.get(
        Uri.parse(
          '${ApiUrl.baseUrl}${ApiUrl.CLUBReferral}?club_id=$clubId',
        ),
        headers: {
      ...
      debugPrint('BaseUrl: ${ApiUrl.baseUrl}${ApiUrl.CLUBReferral}?club_id=$clubId');
      debugPrint('Response status: ${res.statusCode}');
      debugPrint('Response body: ${res.body}');

      if (res.statusCode == 200) {
        final responseData = json.decode(res.body);
        referralModel.value = ReferralModel.fromJson(responseData);
      } else {
