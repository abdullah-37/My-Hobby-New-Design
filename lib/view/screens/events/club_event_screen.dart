...
class ClubEventScreen extends StatefulWidget {
  ...
  ClubEventScreen({super.key, required this.clubId});
...
class _ClubEventScreenState extends State<ClubEventScreen> {
  ...
  @override
  void initState() {
    clubEventController.clubEvent(clubId: widget.clubId);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        ...
        isAction: true,
        onAction: () {
          Get.to(() => CreateEventScreen(clubId: widget.clubId));
        },
      ),
      body: Padding(
        ...
        child: Obx(() {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              return Obx(() {
                final event =
                    clubEventController.clubEventModel.value?.data[index];
                return EventWidget(
                  event: event ?? events[index],
                  onAction: () {
                    debugPrint('profile.id! :: ${clubEventController.user.id!}');
                    Get.to(
                      () => EventDetailsPage(
                        eventDetail: event ?? events[index],
                        userId: clubEventController.user.id!,
                      ),
                    );
                  },
                );
